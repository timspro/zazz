...
//This if statement is to prevent text boxes from being unselectable.
		...
		function updateLayout(changeFromID, changeToID) {
			...
			if (typeof changeFromID !== 'undefined' && typeof changeToID !== 'undefined') {
				array['from'] = changeFromID;
				array['to'] = changeToID;
			}

			$('#-zazz-loader-bar').promote();
			$.post('/zazz/ajax/layout.php', array, function(data) {
		...
		function updateCode(zazz_id, $block, type, codeAction) {
			...
			if (codeAction === $.codeActions.DELETE) {
				...
				if(typeof $.lastUpdate !== 'undefined') {
					...
					$.lastLastUpdateDiv = $.lastUpdateDiv;
				}
				$.lastUpdate = $block;
				$.lastUpdateDiv = $.last_div;
			}
			$('#-zazz-loader-bar').promote();
		...
		$('.-zazz-content-view').mousemove(function(e) {
			...
			var offset_x = Math.round(e.offsetX || e.clientX - $target.offset().left);
			var offset_y = Math.round(e.offsetY || e.clientY - $target.offset().top);
			...
			$('#-zazz-modal-mouse-offsetp').html('<td>Offset (%):</td><td>(' +
					Math.round(offset_y / targetHeight * 1000) / 10 + ',</td><td>' +
					Math.round((targetWidth - offset_x) / targetWidth * 1000) / 10 + ',</td><td>' +
					Math.round((targetHeight - offset_y) / targetHeight * 1000) / 10 + ',</td><td>' +
					Math.round(offset_x / targetWidth * 1000) / 10 + ')</td>');
			$('#-zazz-modal-mouse-locationp').html('<td>Page (%):</td><td>(' +
					Math.round((e.pageY - viewTop) / pageHeight * 1000) / 10 + ',</td><td>' +
					Math.round((bodyWidth - e.pageX) / bodyWidth * 1000) / 10 + ',</td><td>' +
					Math.round((pageHeight - (e.pageY - viewTop)) / pageHeight * 1000) / 10 + ',</td><td>' +
					Math.round(e.pageX / bodyWidth * 1000) / 10 + ')</td>');
		});
		...
		function textareaClick(e) {
			...
			if (18 + myPos.top > e.pageY && e.pageY > myPos.top && myPos.left + 18 > e.pageX &&
					e.pageX > myPos.left && !$this.hasClass('-zazz-css-code')) {
				var $block = $this;
		...
		$('.-zazz-relink-btn').click(function(){
			var id = $.last_div.attr('data-zazz-id');
			confirm('Are you sure?', 'Do you want to remove the code from the current elment (' + 
					id + 
				') and restore the code from the template?', function() {
						$('#-zazz-modal-confirm').hide();
						$('#-zazz-loader-bar').promote();
		...
		$('.-zazz-id-input').blur(function() {
			var last = $.last_div.attr('id');
			var now = $(this).val();
			if(last === 'begin-project' || last === 'end-project' || last === 'begin-page' ||
					last === 'end-page') {
				warn('Error', "Cannot change the ID for element " + last + ".");
				$(this).val(last);
				return;
			}
			if (last !== now) {
				if (typeof $.last_div !== "undefined") {
					if ($('#' + now).length === 0) {
						...
						$.last_div.attr('data-zazz-id', now);
						$('.-zazz-code-block-' + last).each(function() {
							$(this).removeClass('-zazz-code-block-' + last).addClass('-zazz-code-block-' + now);
						});
					} else {
						warn('Error', 'There is already an element with the ID of ' + now);
						$(this).val(last);
					}
				}
				updateLayout(last, now);
			}
		});
		...
		function createDiv(id, type) {
			//If a template was used, there may be something already around with this id.
			var div = $('<div></div>').addClass(type).attr("id", id);
			if (type === "-zazz-element") {
				div.attr('tabindex', '10').attr("data-zazz-id", id);
				if ($('.-zazz-code-block-' + id).length === 0) {
					addCSSCodeBlock(id);
				}
			}
			return div;
		...
		$(document).keyup(function(e) {
			...
			if ((e.keyCode === 117 || e.which === 117 || e.keyCode === 51 || e.which === 51) && e.ctrlKey) {
				$('.-zazz-element[data-zazz-id="begin-project"]').focus();
				$('.-zazz-code-block-begin-project').first().focus();
			}
			if ((e.keyCode === 118 || e.which === 118 || e.keyCode === 52 || e.which === 52) && e.ctrlKey) {
				if(typeof $.lastLastUpdate !== 'undefined') {
					$('.-zazz-element[data-zazz-id="' + $.lastLastUpdateDiv.attr('data-zazz-id') + '"]').focus();
					$.lastLastUpdate.focus();
				}
			}
			if ((e.keyCode === 119 || e.which === 119 || e.keyCode === 53 || e.which === 53) && e.ctrlKey) {
				if(typeof $.lastUpdate !== 'undefined') {
					$('.-zazz-element[data-zazz-id="' + $.lastUpdateDiv.attr('data-zazz-id') + '"]').focus();
					$.lastUpdate.focus();
				}
			}
			if ((e.keyCode === 120 || e.which === 120 || e.keyCode === 54 || e.which === 54) && e.ctrlKey) {
				$('.-zazz-select-btn').focus();
			}
			if ((e.keyCode === 121 || e.which === 121 || e.keyCode === 55 || e.which === 55) && e.ctrlKey) {
				$.last_div.focus();
			}
			if ((e.keyCode === 122 || e.which === 122 || e.keyCode === 56 || e.which === 56) && e.ctrlKey) {
				$('.-zazz-id-input').focus();
			}
			if ((e.keyCode === 123 || e.which === 123 || e.keyCode === 57 || e.which === 57) && e.ctrlKey) {
